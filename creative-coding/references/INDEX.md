...
### `patterns/touchdesigner-pops-depth-geometry.md`
**Added:** 2026-08-29

**Covers:** TOP to POP's Depth mode for converting a depth-map TOP into a 3D point cloud — Camera COMP requirement, Rerange from/to, View Angle Method, Connectivity (including grid-based Quadrilaterals as an alternative to building mesh structure downstream), and why Channel Scope is greyed out in Depth mode. FOV estimation for a roaming performer camera at 5–20ft with no known lens spec (~40° horizontal FOV working default), and why a single static FOV is wrong once a lens is actively zooming. Neighbor POP point-count/performance tuning — the 268k-point frame-drop case at full 518×518 depth resolution, fixed by reducing to 256×256 with High Quality Resize, and the finding that Max Neighbors and Num Hash Buckets were both already near-sensible defaults, meaning raw point count was the actual bottleneck. Ray POP's hard requirement for triangles/quads (never raw points) on its collision input, and that Hit/Reflect need surface normals via Normal POP placed after real mesh connectivity exists. Proximity POP's own independent spatial search (separate from Neighbor's), its Lines-vs-Point-Prims output modes, and why it alone doesn't produce Ray-compatible mesh primitives. A real wrong turn: assuming Neighbor POP's `Nebr` (an index array) could feed Ray POP's direction-expecting Ray Attribute parameter directly — corrected only after checking a reference patch's actual wiring (Ray Attribute was `P`, not `Nebr`; Neighbor/Proximity was a separate branch), not resolvable from documentation prose alone. A composited two-branch architecture (clean silhouette path + separate, arbitrarily-chaotic POP effect path, merged in TOP-land) adopted after a Ray/Noise-collision technique proved to be a poor fit for preserving a live human silhouette. An unresolved "flat wavy rectangle" troubleshooting case with three live hypotheses. **A full per-operator reference section tiers every POP named this session** — four (TOP to POP, Neighbor, Ray, Proximity) have their doc pages read in full and cited; the rest (Connectivity, Normal, Math Combine, Math Mix, Lookup Attribute, Trail, Feedback, Copy, Quantize, Force Radial, Script, Sprinkle, Point Generator) are marked `# UNVERIFIED` — named and discussed, doc pages not opened.

**Use for:** building a depth-camera-to-3D-geometry pipeline in TouchDesigner; diagnosing Neighbor/Proximity/Ray performance problems on a live point cloud; understanding what data actually flows between Neighbor, Ray, and Proximity POPs before wiring them from a tutorial's prose description; deciding whether an effect technique needs to preserve subject shape inside the POP chain or can rely on compositing afterward; checking which POPs from a brainstorm session are doc-confirmed versus still need their reference page read before use.

**Confidence:** Mixed and tiered in place, down to the individual operator — `[Official, read]` for the four fully-read pages, `[Bench]` for the user's own direct observations, `[Designed]` for the composited-architecture decision and all three flat-rectangle hypotheses, `# UNVERIFIED` for every other named POP.

**Open items:** whether Connectivity POP reliably builds clean, normals-ready mesh from Proximity's line output (asserted by the user, not independently verified); whether Num Hash Buckets scaling raises the point-count ceiling past 256×256; the composited two-branch architecture is undesigned-in-detail and unbuilt; root cause of the flat-rectangle appearance, unresolved at session end; whether Lookup Attribute POP's Point Index Units mode is actually the right way to resolve `Nebr` — surfaced late, never tested; every `# UNVERIFIED` operator needs its own page read before its parameters are trusted.
